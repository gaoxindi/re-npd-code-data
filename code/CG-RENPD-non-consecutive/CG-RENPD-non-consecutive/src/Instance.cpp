    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 1; j <= ship_mode_num[i]; j++) {
            ...
            if (j > 1 && ship_time[i][j] <= ship_time[i][j - 1]) {
                throw runtime_error("Shipping times must be strictly increasing in file: " + filename);
            }
        }
    }
...
void Instance::buildModifiedCost() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int t = 1; t <= m; t++) {
            double min_mo_cost = infeasible_cost;
            ...
            for (int j = 1; j <= ship_mode_num[i]; j++) {
                ...
                const double temp_mo_cost = ship_cost[i][j]
