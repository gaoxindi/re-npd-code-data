...
double gapPercent(const SolveResult& result) {
    if (!result.hasFeasibleSolution() || result.lower_bound == 0.0) {
        return 0.0;
    }

    return (result.upper_bound - result.lower_bound) / result.lower_bound * 100.0;
}
...
filesystem::path costAnalysisFilePath(const string& output_file) {
    filesystem::path result_path(output_file);
    filesystem::path parent = result_path.parent_path();
    const string stem = result_path.stem().string();
    const string extension = result_path.extension().empty()
        ? ".txt"
        : result_path.extension().string();
    filesystem::path filename = stem + "-cost" + extension;
...
void writeCostAnalysisResult(ofstream& out, const SolveResult& result) {
    out << result.instance_id << '\t'
        << fixed << setprecision(2)
        << result.transport_cost << '\t'
        << result.early_tardy_cost << '\t'
...
int main(int argc, char* argv[]) {
    ...
    const filesystem::path cost_analysis_file_path = costAnalysisFilePath(output_file);
    ofstream cost_analysis_file(cost_analysis_file_path);
    if (!cost_analysis_file.is_open()) {
        cerr << "Error: cannot open cost analysis file "
            << cost_analysis_file_path.string() << endl;
        return 1;
    }
    ...
    try {
        for (int instance_id = start_instance; instance_id <= end_instance; instance_id++) {
            ...
            if (result.hasFeasibleSolution()) {
                writeCostAnalysisResult(cost_analysis_file, result);
            }
        }
    }
    catch (exception& e) {
